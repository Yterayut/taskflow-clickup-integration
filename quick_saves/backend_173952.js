...
function filterDataByRole(data, role, userEmail) {
    ...
    if (role === 'Manager') {
                ...
                if (hasEmail) {
                    ...
                    if (totalAssignees > 1) {
                        const otherAssignees = task.assignees
                            .filter(assignee => assignee.email && assignee.email.toLowerCase() !== userEmailLower)
        ...
        // Filter subtasks: ONLY assigned to this user (email-only matching)
        const filteredSubtasks = (data.subtasks || []).filter(subtask => {
            if (subtask.assignees && subtask.assignees.length > 0) {
                // 🎯 EMAIL-ONLY MATCHING: Use only email for unique identification
                const hasEmail = subtask.assignees.some(assignee => 
                    assignee.email && assignee.email.toLowerCase() === userEmailLower
                );
