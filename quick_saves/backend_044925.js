...
function filterDataByRole(data, role, userEmail) {
    ...
    if (role === 'Manager') {
        ...
        const firstName = currentUser.name.split(' ')[0].toLowerCase(); // Define firstName here
        
        // ENHANCED DEBUG: Check ClickUp assignee data structure for multi-assignee tasks
        console.log(`🔍 DEBUGGING MULTI-ASSIGNEE STRUCTURE:`);
        console.log(`🔍 Total tasks to check: ${(data.tasks || []).length}`);
        console.log(`🔍 Employee: ${currentUser.name} (${userEmail})`);
        console.log(`🔍 Looking for tasks where this employee is one of the assignees`);
        
        // Check first 5 tasks for multi-assignee structure
        (data.tasks || []).slice(0, 5).forEach((task, index) => {
            console.log(`🔍 Task ${index + 1}: "${task.name}"`);
            console.log(`🔍   Total assignees: ${task.assignees ? task.assignees.length : 0}`);
            
            if (task.assignees && task.assignees.length > 0) {
                task.assignees.forEach((assignee, idx) => {
                    console.log(`🔍   Assignee ${idx + 1}:`, {
                        username: assignee.username || 'NO_USERNAME',
                        email: assignee.email || 'NO_EMAIL',
                        isTargetUser: assignee.email && assignee.email.toLowerCase() === userEmailLower
                    });
                });
            } else {
                console.log(`🔍   No assignees found (Unassigned task)`);
            }
        });
        ...
        const filteredTasks = (data.tasks || []).filter(task => {
            ...
            if (task.assignees && task.assignees.length > 0) {
                ...
                // 🎯 EMAIL-ONLY MATCHING: Use only email for unique identification
                const hasEmail = task.assignees.some(assignee => {
                    // Check both email and username fields for more robust matching
                    const emailMatch = assignee.email && assignee.email.toLowerCase() === userEmailLower;
                    const usernameMatch = assignee.username && assignee.username.toLowerCase() === userEmailLower;
                ...
                if (hasEmail) {
                    ...
                    const matchedAssignee = task.assignees.find(assignee => {
                        const emailMatch = assignee.email && assignee.email.toLowerCase() === userEmailLower;
                        const usernameMatch = assignee.username && assignee.username.toLowerCase() === userEmailLower;
                        return emailMatch || usernameMatch;
                    });
                    
                    console.log(`👤✅ Task "${task.name}" matched for ${currentUser.name}`);
                    console.log(`👤   Multi-assignee task: ${totalAssignees} assignees total`);
                    console.log(`👤   User is assignee: ${matchedAssignee?.username || matchedAssignee?.email}`);
                    ...
                    if (totalAssignees > 1) {
                        const otherAssignees = task.assignees
                            ...
                            .join(', ');
                        console.log(`👤   Other assignees: ${otherAssignees}`);
                    }
        ...
        // Filter subtasks: ONLY assigned to this user (email and username matching)
        const filteredSubtasks = (data.subtasks || []).filter(subtask => {
            if (subtask.assignees && subtask.assignees.length > 0) {
                // 🎯 ENHANCED MATCHING: Use both email and username for robust identification
                const hasMatch = subtask.assignees.some(assignee => {
                    const emailMatch = assignee.email && assignee.email.toLowerCase() === userEmailLower;
                    const usernameMatch = assignee.username && assignee.username.toLowerCase() === userEmailLower;
...
app.get('/api/v1/dashboard', async (req, res) => {
    ...
    try {
        ...
        const MAX_TASKS_PER_TEAM = 500; // เพิ่ม limit เพื่อให้ได้ tasks มากขึ้น
        const MAX_PAGES_PER_TEAM = 5;   // เพิ่ม pagination limit
        ...
        const teamPromises = teams.map(async (team) => {
            try {
                ...
                let teamMembers = [];
                // NO CACHE - Direct API call for team members
                teamMembers = team.members || [];
                
                // If no members in team data, fetch them separately (optional)
                if (teamMembers.length === 0) {
                    try {
                        const membersResponse = await callClickUpAPI(`/team/${team.id}/member`);
                        teamMembers = membersResponse.members || [];
                    } catch (err) {
                        console.log(`No members endpoint for team ${team.id}:`, err.message);
                    }
                } 
                ...
                while (hasMore && page < MAX_PAGES_PER_TEAM) {
                    try {
                        // Remove date filter to get ALL tasks (not just recent ones)
                        const tasksData = await callClickUpAPI(
                            `/team/${team.id}/task?archived=false&include_closed=true&page=${page}&order_by=updated&reverse=true`
                        );
        ...
        const filteredTasks = filteredRawData.tasks || [];
        const filteredSubtasks = filteredRawData.subtasks || [];
        const filteredTasksAndSubtasks = [...filteredTasks, ...filteredSubtasks];
        
        // 🎯 ROLE-BASED TASK COUNTING:
        // Manager/Team Lead: Show all (main + subtasks)
        // Employee: Show only assigned tasks (main + subtasks that are assigned to them)
        
        const showAllTasksAndSubtasks = filteredTasksAndSubtasks; // For display
        const filteredTotalTasks = showAllTasksAndSubtasks.length; // Count all filtered tasks
        const filteredCompletedTasks = showAllTasksAndSubtasks.filter(task => 
            task.status?.status === 'complete' || task.status?.status === 'closed'
        ).length;
        ...
        const finalResponse = {
            ...
            inProgressTasks: showAllTasksAndSubtasks.filter(task => 
                task.status?.status === 'in progress' || task.status?.status === 'in review'
            ).length,
            todoTasks: showAllTasksAndSubtasks.filter(task => 
                task.status?.status === 'to do' || task.status?.status === 'Open'
            ).length,
            overdueTasks: showAllTasksAndSubtasks.filter(task => {
                const dueDate = task.due_date ? new Date(parseInt(task.due_date)) : null;
                return dueDate && dueDate < new Date() && task.status?.status !== 'complete';
            }).length,
            ...
            mainTasks: filteredTasks.length,
            subtasks: filteredSubtasks.length,
            tasks: showAllTasksAndSubtasks.map(task => {
                // For Employee role: Show user as primary assignee if they're assigned to the task
                let displayAssignee = task.assignees?.[0]?.username || 'Unassigned';
                let displayEmail = task.assignees?.[0]?.email || null;
                
                if (sessionData.role === 'Employee' && task.assignees && task.assignees.length > 0) {
                    // Find if current user is in assignees
                    const userAssignee = task.assignees.find(assignee => 
                        assignee.email && assignee.email.toLowerCase() === sessionData.email.toLowerCase()
                    );
                    
                    if (userAssignee) {
                        // Show current user as primary assignee
                        displayAssignee = userAssignee.username || userAssignee.email;
                        displayEmail = userAssignee.email;
                        
                        // Add indicator for multi-assignee tasks
                        if (task.assignees.length > 1) {
                            displayAssignee += ` (+${task.assignees.length - 1} others)`;
                        }
                    }
                }
                ...
                return {
                    ...
                    assigneeEmail: displayEmail,
                    totalAssignees: task.assignees?.length || 0,
                    due_date: task.due_date,
                    url: task.url,
                    isSubtask: filteredSubtasks.some(st => st.id === task.id)
                };
            }),
            team: uniqueMembers.map(member => ({
                role: 'Member',
...
app.post('/api/v1/cache/refresh', async (req, res) => {
    ...
    try {
        ...
        // NO CACHE - Cache references removed
        console.log('Cache cleared (no-cache mode)');
...
app.get('/api/v1/employees', (req, res) => {
    ...
    dbHelpers.getEmployees((err, rows) => {
        if (err) {
...
app.post('/api/v1/employees', (req, res) => {
    ...
    const { email, name, skills, role } = req.body;
    
    if (!email || !name) {
        return res.status(400).json({ success: false, error: 'Email and name are required' });
    }

    dbHelpers.upsertEmployee(email, name, skills || '', role || 'Employee', (err) => {
        if (err) {
            console.error('Database error:', err);
            return res.status(500).json({ success: false, error: 'Database error' });
        }
...
app.get('/api/v1/leaderboard', (req, res) => {
    ...
    dbHelpers.getTaskScores((err, rows) => {
        if (err) {
            console.error('Database error:', err);
            return res.status(500).json({ success: false, error: 'Database error' });
        }
...
app.post('/api/v1/task-scores', (req, res) => {
    const sessionId = req.headers.authorization?.replace('Bearer ', '');
    ...
    const { taskId, taskName, employeeEmail, qualityScore, complexityFactor, timeBonus, teamworkPoints, penalty } = req.body;
    
    if (!taskId || !employeeEmail || !qualityScore) {
        return res.status(400).json({ success: false, error: 'Task ID, employee email, and quality score are required' });
    }

    dbHelpers.addTaskScore(
        taskId, taskName || '', employeeEmail, 
        parseInt(qualityScore), parseFloat(complexityFactor) || 1.0, 
        parseInt(timeBonus) || 0, parseInt(teamworkPoints) || 0, parseInt(penalty) || 0,
        (err) => {
            if (err) {
                console.error('Database error:', err);
                return res.status(500).json({ success: false, error: 'Database error' });
            }
            res.json({ success: true, message: 'Task score added successfully' });
        }
    );
});
...
app.get('/api/v1/attendance', (req, res) => {
    ...
    dbHelpers.getAttendance(startDate, endDate, (err, rows) => {
        if (err) {
            console.error('Database error:', err);
            return res.status(500).json({ success: false, error: 'Database error' });
        }
...
app.post('/api/v1/attendance', (req, res) => {
    ...
    const { employeeEmail, employeeName, date, checkIn, checkOut, leaveType, leaveRef, leaveReason, status } = req.body;
    
    if (!employeeEmail || !date || !status) {
        return res.status(400).json({ success: false, error: 'Employee email, date, and status are required' });
    }

    dbHelpers.addAttendance(
        employeeEmail, employeeName || '', date, checkIn, checkOut, 
        leaveType, leaveRef, leaveReason, status,
        (err) => {
            if (err) {
                console.error('Database error:', err);
                return res.status(500).json({ success: false, error: 'Database error' });
            }
            res.json({ success: true, message: 'Attendance record saved successfully' });
        }
    );
});

// Leaderboard Settings APIs
app.get('/api/v1/leaderboard-settings', (req, res) => {
    const sessionId = req.headers.authorization?.replace('Bearer ', '');
    ...
    dbHelpers.getLeaderboardSettings((err, rows) => {
        if (err) {
            console.error('Database error:', err);
            return res.status(500).json({ success: false, error: 'Database error' });
        }
...
app.put('/api/v1/leaderboard-settings/:settingName', (req, res) => {
    const sessionId = req.headers.authorization?.replace('Bearer ', '');
    ...
    dbHelpers.updateLeaderboardSetting(settingName, settingValue, sessionData.email, (err) => {
        if (err) {
            console.error('Database error:', err);
            return res.status(500).json({ success: false, error: 'Database error' });
        }
...
// Export Attendance Data API
app.get('/api/v1/attendance/export', (req, res) => {
    const sessionId = req.headers.authorization?.replace('Bearer ', '');
    ...
    dbHelpers.getAttendance(startDate, endDate, (err, rows) => {
        ...
        if (format === 'csv') {
            // Generate CSV
            const csvHeader = 'Employee Name,Email,Date,Check In,Check Out,Status,Leave Type,Leave Reference,Leave Reason\n';
            const csvData = rows.map(row => 
                `"${row.employee_name}","${row.employee_email}","${row.date}","${row.check_in_time || ''}","${row.check_out_time || ''}","${row.status}","${row.leave_type || ''}","${row.leave_reference || ''}","${row.leave_reason || ''}"`
            ).join('\n');
...
function authenticateToken(req, res, next) {
    ...
    // Get user info
    const user = usersConfig.users.find(u => u.email === session.userEmail);
    if (!user) {
        return res.status(401).json({
            success: false,
...
app.get('/api/v1/tasks', authenticateToken, async (req, res) => {
    try {
        ...
        // Get ClickUp data for the user
        const clickupData = await getClickUpDataForUser(req.user.email, req.user.role);
        
        res.json({
            success: true,
            tasks: clickupData.tasks || [],
            lists: clickupData.lists || [],
            total: clickupData.totalTasks || 0
        });
...
app.post('/api/v1/tasks', authenticateToken, async (req, res) => {
    try {
        ...
        const { name, list_id, priority, status, due_date, description, assignees } = req.body;
        
        if (!name || !list_id) {
            return res.status(400).json({
                success: false,
                error: 'Task name and list are required'
            });
        }
        ...
        const taskData = {
            ...
            assignees: assignees ? assignees.map(a => ({ email: a.email })) : [],
            status: status || 'to do'
        };

        console.log('Creating task in ClickUp:', taskData);
        
        // For now, simulate success (actual ClickUp API would be called here)
        res.json({
            success: true,
...
// Update task
app.put('/api/v1/tasks/:taskId', authenticateToken, async (req, res) => {
    try {
        if (!masterClickUpToken) {
            return res.status(503).json({
                success: false,
        ...
        console.log('Updating task in ClickUp:', taskId, taskData);
...
app.delete('/api/v1/tasks/:taskId', authenticateToken, async (req, res) => {
    try {
        ...
        console.log('Deleting task in ClickUp:', taskId);
...
app.listen(PORT, '0.0.0.0', () => {
    ...
    console.log(`📋 Database APIs enabled:`);
