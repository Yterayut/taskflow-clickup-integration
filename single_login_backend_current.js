...
const { AccountSecurityService } = require('./infrastructure/adapters/AccountSecurityService');
const { AuditLoggingService } = require('./infrastructure/adapters/AuditLoggingService');
...
async function initializeServices() {
    try {
        ...
        const accountSecurityService = new AccountSecurityService(dbClient);
        const auditLoggingService = new AuditLoggingService(dbClient);
        
        // Initialize Cache Service
        console.log('🔄 Initializing Cache Service (Redis)...');
        ...
        const backgroundSyncService = new BackgroundSyncService({
            clickupService: clickupIntegration,
        ...
        app.set('accountSecurityService', accountSecurityService);
        app.set('auditLoggingService', auditLoggingService);
        app.set('backgroundSyncService', backgroundSyncService);
...
// Health check endpoint with token encryption status
app.get('/health', async (req, res) => {
    try {
        ...
        // Check token encryption service health
        const { tokenEncryptionService } = require('./infrastructure/adapters/TokenEncryptionService');
        let encryptionHealth = null;
        
        try {
            encryptionHealth = await tokenEncryptionService.healthCheck();
        } catch (error) {
            encryptionHealth = { status: 'unhealthy', error: error.message };
        }
...
// OLD: ClickUp API direct calls (misleading route name)
// app.use('/api/v2/local', require('./api/routes/localDataRoutes'));

// LEGACY: Local Database + Cache optimized routes (transitional)
app.use('/api/v2/local', require('./api/routes/localDataOptimizedRoutes'));

// NEW: Dashboard routes with correct naming and source labels
app.use('/api/v2/dashboard', require('./api/routes/dashboardOptimizedRoutes'));
...
app.use('/api/v2/realtime', require('./api/routes/realtimeRoutes')(realtimeService));
...
app.use('/api/*', (req, res) => {
    res.status(404).json({
        ...
        available_endpoints: {
            ...
            v2_clickup: '/api/v2/clickup/*',
            v2_local: '/api/v2/local/*',
            oauth: '/auth/clickup/*'
        }
    });
});
...
async function startServer() {
    try {
        ...
        // Check initial system status (with error handling)
        ...
        try {
            ...
            console.warn('⚠️  System status check failed, server will continue:', statusError.message);
            console.log('📊 System running in degraded mode - some features may be limited');
        }
        
        // Start Background Sync Service
        console.log('🔄 Starting Background Sync Service...');
        try {
            const backgroundSyncService = app.get('backgroundSyncService');
            await backgroundSyncService.start();
            console.log('✅ Background Sync Service started successfully');
        } catch (syncError) {
            console.warn('⚠️  Background Sync Service failed to start:', syncError.message);
            console.log('📊 System will continue without background sync - manual sync available');
        }
        ...
        const server = app.listen(PORT, async () => {
            ...
            console.log(`📋 Audit API: http://localhost:${PORT}/api/v2/audit/`);
            
            // Start realtime service
            try {
                await realtimeService.initialize();
                console.log(`📡 Realtime service running on port 7813`);
                console.log(`🔌 WebSocket endpoint: ws://localhost:7813`);
            } catch (error) {
            ...
            try {
                const auditService = app.get('auditLoggingService');
                if (auditService) {
                    await auditService.logEvent({
                        eventType: auditService.eventTypes.SYSTEM_STARTUP,
                        description: `TaskFlow Pro backend started on port ${PORT}`,
                        riskLevel: auditService.riskLevels.LOW,
                        eventData: {
                            port: PORT,
                            version: '2.1.1-enhanced',
                            features: ['Security', 'Audit Logging', 'Analytics']
                        }
                    });
                }
            } catch (error) {
                console.warn('⚠️  Failed to log startup event:', error.message);
            }
