...
const usersConfig = JSON.parse(fs.readFileSync('users_config.json', 'utf8'));
...
const CLICKUP_CONFIG = {
    ...
    REDIRECT_URI: 'http://192.168.20.10:7810/',
...
// Authentication endpoints for the login page
app.post('/api/auth/login', (req, res) => {
    ...
    const user = usersConfig.users.find(u => u.email.toLowerCase() === email.toLowerCase() && u.password === password);
    ...
    res.json({
        ...
        token: 'session_' + Date.now(), // Simple token for compatibility
        expiresIn: '24h'
    });
});
...
app.get('/api/auth/verify', (req, res) => {
    ...
    const user = usersConfig.users.find(u => u.email === req.session.userId);
    if (!user) {
        return res.status(401).json({
            success: false,
    ...
    res.json({
        ...
        clickupConnected: userTokens.has(user.email)
    });
});
...
app.get('/auth/clickup', (req, res) => {
    ...
    // Test 1: Minimal OAuth without scope (as per original working config)
    ...
    console.log(`[${new Date().toISOString()}] Redirecting to ClickUp OAuth (MINIMAL): ${authUrl}`);
    console.log(`[${new Date().toISOString()}] OAuth State: ${state}`);
    console.log(`[${new Date().toISOString()}] Redirect URI: ${CLICKUP_CONFIG.REDIRECT_URI}`);
...
// OAuth Step 2: Handle callback and exchange code for token (ClickUp redirects to root)
app.get('/', async (req, res) => {
    // Check if this is OAuth callback (has code parameter)
    if (!req.query.code) {
        // Not OAuth callback, show basic info
        return res.json({
            status: 'TaskFlow Pro OAuth Service',
            message: 'ClickUp OAuth callback endpoint ready',
            timestamp: new Date().toISOString()
        });
    }
    ...
    console.log(`[${new Date().toISOString()}] Callback state: ${state}`);
    console.log(`[${new Date().toISOString()}] Callback error: ${error || 'NONE'}`);
    
    if (error) {
        console.log(`[${new Date().toISOString()}] OAuth error: ${error}`);
    ...
    if (req.session.oauth_state && state !== req.session.oauth_state) {
        console.log(`[${new Date().toISOString()}] OAuth state mismatch: expected ${req.session.oauth_state}, got ${state}`);
        return res.redirect('http://192.168.20.10:8888?error=Invalid state parameter');
    }
...
app.get('/api/v2/local/dashboard-data', async (req, res) => {
    ...
    try {
        ...
        // Get fresh data from ClickUp API (reuse existing logic)
        const teamsData = await callClickUpAPI('/team', userId);
        ...
        // Call existing comprehensive data fetching logic
        let allSpaces = [];
        let allFolders = [];
        let allLists = [];
