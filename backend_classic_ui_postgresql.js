// TaskFlow Pro - Classic UI Compatible PostgreSQL Backend
// Version: v13.1.0-classic-ui-compatible
// Date: 2025-08-10
// Real ClickUp data only - No demo/mock data
...
app.get('/api/v2/dashboard/analytics', async (req, res) => {
    try {
        // Classic UI bypass: Create default session if not exists
        if (!req.session.user) {
            req.session.user = {
                id: "classic_ui_user",
...
startServer();// Classic UI compatibility endpoint - Real ClickUp data from PostgreSQL
app.get("/api/v1/production/clickup-data", async (req, res) => {
    try {
        ...
        console.error("Production ClickUp data error:", error);
        res.status(500).json({
