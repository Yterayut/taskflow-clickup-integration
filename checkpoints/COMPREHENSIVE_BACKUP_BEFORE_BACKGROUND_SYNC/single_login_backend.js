...
// Regular User Authentication - Email/Password Login
app.post('/api/v2/auth/login', (req, res) => {
    const { email, password } = req.body;
    
    console.log(`[${new Date().toISOString()}] Regular user login attempt: ${email}`);
    ...
    // Check if user should use OAuth
    if (user.auth_type === 'oauth') {
        console.log(`[${new Date().toISOString()}] OAuth user attempted regular login: ${email}`);
        return res.json({
            status: 'oauth_setup_required',
            message: 'This account requires OAuth authentication',
            redirectUrl: '/auth/clickup'
        });
    }
    ...
    console.log(`[${new Date().toISOString()}] Regular user login successful: ${email} (${user.role})`);
...
server.listen(PORT, () => {
    console.log(`🚀 TaskFlow Backend (PHASE 3 REAL-TIME) running on port ${PORT}`);
    ...
    console.log(`🎯 Enhanced Features (PHASE 3):`);
    console.log(`   - 🔴 REAL-TIME WebSocket notifications`);
    console.log(`   - 🔴 Live team collaboration & messaging`);
    console.log(`   - 🔴 Instant task updates & status changes`);
    console.log(`   - 🔴 Role-based real-time rooms`);
    console.log(`   - 📊 Advanced analytics & performance tracking`);
    console.log(`   - 🔄 Auto-refresh dashboard components`);
    console.log(`   - 👥 Live user presence & typing indicators`);
    console.log(`   - 📈 Real-time productivity metrics`);
    console.log(`   - 🎯 Enhanced user experience with instant updates`);
    console.log(`   - 💬 Team messaging & collaboration tools`);
    console.log(`   - 🔔 Push notifications for important events`);
    console.log(`   - 📱 Mobile-optimized real-time interface`);
});
