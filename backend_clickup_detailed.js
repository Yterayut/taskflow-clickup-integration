...
app.get('/health', (req, res) => {
    res.json({
        ...
        service: 'TaskFlow Backend - ClickUp Integration Detailed',
        version: '3.2.0-detailed-debug',
        clickup: {
            configured: true,
...
// OAuth callback handler - FIXED WITH DETAILED LOGGING
app.get('/api/v1/auth/clickup/callback', async (req, res) => {
    ...
    try {
        ...
        // Method 1: Standard OAuth2 token exchange
        try {
            log('info', 'Attempting token exchange method 1: Standard OAuth2');
            ...
            // Method 2: Form-encoded token exchange
            try {
                log('info', 'Attempting token exchange method 2: Form-encoded');
                
                const params = new URLSearchParams();
                params.append('client_id', CLICKUP_CONFIG.clientId);
                params.append('client_secret', CLICKUP_CONFIG.clientSecret);
                params.append('code', code);
                
                tokenResponse = await axios.post('https://app.clickup.com/api/v2/oauth/token', params, {
                    headers: {
                ...
                log('error', 'Token exchange method 2 failed', method2Error.response?.data || method2Error.message);
                
                // Method 3: Alternative endpoint
                try {
                    log('info', 'Attempting token exchange method 3: Alternative endpoint');
                    
                    tokenResponse = await axios.post('https://api.clickup.com/api/v2/oauth/token', {
                        client_id: CLICKUP_CONFIG.clientId,
                        client_secret: CLICKUP_CONFIG.clientSecret,
                        code: code,
                    ...
                    log('info', 'Token exchange response method 3', { 
                        status: tokenResponse.status,
                        hasData: !!tokenResponse.data,
                        dataKeys: Object.keys(tokenResponse.data || {})
                    });
...
app.post('/api/v1/test/token-exchange', async (req, res) => {
    const { code } = req.body;
    ...
    try {
        log('info', 'Testing token exchange', { code: code.substring(0, 10) + '...' });
        ...
        log('error', 'Token exchange test failed', error.response?.data || error.message);
        res.status(500).json({
            success: false,
...
app.listen(PORT, '0.0.0.0', () => {
    log('info', '🚀 TaskFlow Pro Backend (Detailed Debug) started', {
        port: PORT,
        clientId: CLICKUP_CONFIG.clientId,
        redirectUri: CLICKUP_CONFIG.redirectUri
    });
    ...
    console.log(`🔧 OAuth Enhanced: Multiple token exchange methods with detailed logging`);
});
