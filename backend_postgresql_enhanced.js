...
app.post('/api/v2/auth/login', async (req, res) => {
    try {
        ...
        let user = null;
        try {
            const users = await DatabaseAdapter.query('SELECT * FROM users WHERE email = ? AND is_active = ?', [email, 1]);
            user = users[0];
        } catch (error) {
        ...
        try {
            ...
            const analytics = {
                ...
                recentActivities: recentTasks.map(task => ({
                    description: `${task.name.substring(0, 50)}${task.name.length > 50 ? '...' : ''}`,
                    status: task.status,
                    timestamp: task.updated_at,
                    type: 'task_update',
                    task_id: task.id
                })),
                dataSource: 'Real ClickUp Data',
                database: DatabaseAdapter.getType(),
                lastUpdate: new Date().toISOString(),
                team_details: members.map(m => ({
                    name: m.username,
                    email: m.email,
                    tasks_assigned: tasks.filter(t => {
                        try {
                            const assignees = JSON.parse(t.assignees || '[]');
                            return assignees.some(a => a.id === m.id);
                        } catch {
                            return false;
                        }
                    }).length
            ...
            res.json({
                success: true,
                data: {
                    message: "Database error - no real data available",
                    dataSource: "Database Error - Real Data Not Available",
                    totalTasks: 0,
                    completedTasks: 0,
                    inProgressTasks: 0,
                    teamMembers: 0,
...
app.get('/api/v2/tasks/my-tasks', async (req, res) => {
    try {
        ...
        try {
            const tasks = await DatabaseAdapter.query(`
                SELECT 
                    t.*,
                    JSON_EXTRACT(t.assignees, '$') as task_assignees
                FROM clickup_tasks t 
                WHERE JSON_EXTRACT(t.assignees, '$') LIKE ? 
                   OR t.id IN (
                       SELECT task_id FROM task_assignments WHERE user_email = ?
                   )
            ...
            res.json({
                ...
                data: {
                    ...
                    total_count: 0,
                    user_email: userEmail,
                    data_source: "No Real Data - Sync Required",
                    error: "Database schema not ready for real ClickUp data"
                }
            });
        }
...
app.get('/api/v2/team/overview', async (req, res) => {
    try {
        ...
        try {
            ...
            const members = await DatabaseAdapter.query('SELECT * FROM clickup_members');
            const tasks = await DatabaseAdapter.query('SELECT status, assignees FROM clickup_tasks');
...
app.get('/api/v2/projects', async (req, res) => {
    try {
        ...
        try {
            ...
            const tasks = await DatabaseAdapter.query('SELECT list_id, status FROM clickup_tasks');
